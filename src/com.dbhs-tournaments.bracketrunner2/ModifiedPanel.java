...
/**
 * Panel used to visually represent a single modified format
 * tournament bracket.
 * 
...
class ModifiedPanel extends BracketPanel {
	 ...
	 */
	public ModifiedPanel(int fullBracket) {
		
		super(bf.createBracket(fullBracket, BracketFactory.MODIFIED));
	...
	void drawLosersMatch(Graphics g, Match match) {
		...
		x = 3*matchLength + 2*connectorLength - (match.getRound()-1)*(matchLength+connectorLength) + endLength;
		...
		if(prizes.length > totalRounds - match.getRound() + 1){
	    	g.drawString("$" + Integer.toString(prizes[totalRounds - match.getRound() + 1]), x-endLength*37/8, y-endLength*11/8);
	    }
	...
	void drawWinnersMatch(Graphics g, Match match) {
		...
		x = (match.getRound()-1)*(matchLength+connectorLength) + endLength*37;
		if(match.getRound() == 4) {
